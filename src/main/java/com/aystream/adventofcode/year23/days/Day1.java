package com.aystream.adventofcode.year23.days;

import com.aystream.adventofcode.year23.AdventOfCode2023;
...
public class Day1 extends AdventOfCode2023 {
    ...
    public Object part1() {
        return dayStream()
                .map(s -> firstDigit(s).getRight() + lastDigit(s).getRight())
    ...
    private static Stream<Pair<Integer, String>> pairStream(String s) {
        return IntStream.range(0, s.length())
                .mapToObj(i -> Pair.of(i, s.charAt(i) + ""))
                .filter(x -> Character.isDigit(x.getRight().charAt(0)));
    }

    @Override
    public Object part2() {
        return dayStream()
                .map(s -> firstStringDigit(s) + lastStringDigit(s))
                .mapToInt(Integer::parseInt).sum();
    }

    private String firstStringDigit(String s) {
        var pair = nums.keySet().stream()
                .map(x -> Pair.of(x, s.indexOf(x)))
                .filter(x -> x.getRight() != -1)
                .min(Comparator.comparingInt(Pair::getRight)).orElse(Pair.of("", Integer.MAX_VALUE));
    ...
    private String lastStringDigit(String s) {
        var pair = nums.keySet().stream()
                .map(x -> Pair.of(x, s.lastIndexOf(x)))
                .filter(x -> x.getRight() != -1)
                .min((x, y) -> y.getRight() - x.getRight()).orElse(Pair.of("", Integer.MIN_VALUE));
